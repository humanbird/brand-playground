...
- Refresh mode hardened after a real refresh of a pre-provenance kit: legacy stamp recovery, scaffold-contract check with an additive alias block, create-if-missing support files, package.json merge rule, refresh verification list
- Meta phase is generated by `scripts/gen-components-meta.mjs` instead of hand-written (fixes headless runs stalling mid-phase)
