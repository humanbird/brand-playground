...
| `{{SECTION: instructions}}` | `kit-common/SKILL.md.template` (`STACK`, `CODE_FIRST`, `ICONS`, `ANATOMY`, `LAYOUT`, `TOKENS`, `COMPONENT_CHOICE`, `STANCE`, `WIDTH_NOTE`, `GREP_EXCEPTIONS`), `AGENTS.md.template` (`STACK`), `pitfalls.md.template` (`PITFALLS`) | Prose blocks the generator writes from ingestion evidence; the instruction text inside the braces is replaced entirely. For this template, `STACK` means: replace every React/`.tsx` reference with the Vue equivalents from "Differences from kit-react" below |
...
| `design/tokens.css` | The frozen tokens from the target design system. Replace the entire file, but **keep the generic `--ds-*` aliases defined** (`--ds-color-canvas`, `-surface`, `-line`, `-ink`, `-ink-muted`, `-accent`, `-accent-hover`, `-on-accent`, `-success`, `-danger`, spacing-unit, radii, fonts, font sizes, weights, line heights, border width, shadows—the exact list is the SCAFFOLD CONTRACT comment at the top of the template's `tokens.css`)—map them to the DS tokens with `var()`; never drop or rename them, because `src/kit.css` and the scaffold resolve to them directly. If the design system supplies its own CSS variables (see "Design Systems with Their Own CSS" below), this file contains only those mappings. |
...
| `pnpm export` | static **single-file** export of all prototypes to `export/index.html`—runs in a browser when opened directly, hence hash history and `base: './'`. `pnpm export --only <slug>` exports only `src/prototypes/<slug>/` (plus its `_shared/`) and opens it directly at its declared viewport—mechanism: `VITE_ONLY` env var, read by the `kit:only-prototype` plugin in `vite.config.ts`, which narrows the discovery globs before Vite expands them |

If another kit on the same machine already uses the port, assign the new kit its own
port in `vite.config.ts`, `.claude/launch.json`, **and** the `{{PORT}}` placeholder;
otherwise the agent will see the wrong server.
...
| Shared shell | `_shared/Shell.tsx` | `_shared/Shell.vue` | same rule: a shell file exports the component only |
...
2. Retain `src/styles.css`, but remove `@import "tailwindcss"` and `@theme`; only the base
   ...
   classes remain.
3. Import the design system's base stylesheet in `src/main.ts` **before** the kit
...
4. `design/tokens.css` does **not** duplicate the design system's variables. It documents
   the semantic groups, refers to the package source, and bridges the generic `--ds-*`
   aliases to the package variables with `var()`; the package remains the source of truth,
   preventing the kit from silently diverging after the next version upgrade.
5. The scaffold files (`HomeView.vue`, `NotFound.vue`) use only `kit-*` classes from
   `src/kit.css`, which resolve against the `--ds-*` aliases. With the bridge from step 4 in
   place they need no change—and they must not be changed, or refresh cannot re-copy them.

The template supports either path: all Tailwind coupling is contained in `vite.config.ts`
and `src/styles.css`—and nowhere else.
...
- **Font sizes** use Tailwind names in the template (`--text-xs/sm/base/lg/xl/2xl`). If the
  target design system defines its own step names, the generator may adopt them in
  `src/styles.css`—but then exclusively; two naming systems side by side make the prototype
  agent guess for every class. The scaffold files are unaffected either way.
...
- `src/HomeView.vue`, `src/NotFound.vue`, `src/App.vue`—scaffold. Never maintained by hand.
- `src/kit.css`—the `@layer kit` rules the scaffold files use, resolved against `--ds-*` aliases.
- `src/viewport.ts`, `src/DeviceFrame.vue`—the desktop/mobile viewport toggle on the home page.
...
- `src/main.ts`—stylesheet order is significant and fixed:
  ...
  fonts, tokens, styles, and fixes; it does not change the imports. **Exception:** if the design system
  supplies its own base stylesheet, import it first (see above); refresh then shows
  `main.ts` as a diff instead of re-copying it.
...
- `package.json` (name, pinned DS packages, Tailwind removed or not), `vite.config.ts` (port,
  Tailwind plugin), `.claude/launch.json` (name, port), `index.html` (title, theme attributes).

Never touched by refresh: everything under `design/`, `src/components/`, `src/icons/icons.ts`,
