...
1. Confirm the target directory with the user (default: `~/dev/<ds>-rapidprototype`), copy `templates/kit-react/` there, and replace placeholders according to `TEMPLATE.md` (including the port if it differs: keep vite.config.ts, `.claude/launch.json`, and the CLAUDE.md command table in sync). Remove TEMPLATE.md; keep the example/ prototype until the first real prototype exists, then delete it. If the DS code uses Vue or another framework, present the template choice to the user.
