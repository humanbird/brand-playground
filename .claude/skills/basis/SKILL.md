---
name: basis
description: Generates a runnable prototyping kit repository from design system input (code library, Figma, live website, specifications, screenshots, or any combination) following the RapidPrototype pattern. It is used in this repository whenever a design foundation needs to be created or refined (/basis, “build foundation,” “generate kit,” new DS input).
---
...
1. Confirm the target directory with the user (default: `~/dev/<ds>-rapidprototype`), copy `templates/kit-react/` there, and replace placeholders according to `TEMPLATE.md` (including the port if it differs: keep vite.config.ts, `.claude/launch.json`, and the CLAUDE.md command table in sync). Then remove `TEMPLATE.md` and the `example/` prototype from the kit—generator documentation does not belong in the consumer repository. If the DS code uses Vue or another framework, present the template choice to the user.
...
Build a representative reference screen under `src/prototypes/referenz-<name>/` and inspect it in the browser (typecheck, console, realistic width). What it shows depends on the source:
...
Validated by the Onyx field test—the workflow becomes shorter, and several contract points change roles:
...
- **Keep Tailwind or remove it?** Base this decision on what the DS provides for layout. Onyx provides its own grid and utilities, so Tailwind was removed. Primer 38 provides neither `Box` nor `sx`, only components, so Tailwind remains—but explicitly as a *layout* tool, with breakpoints aligned to the DS tiers and a rule that colors/typography never come from Tailwind. Explain the decision in CLAUDE.md and the skill.
