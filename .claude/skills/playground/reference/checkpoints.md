...
Note on the meta phase: it is generated by `scripts/gen-components-meta.mjs`, never hand-written — a headless run that hand-writes the whole file in one step tends to die mid-write and leaves no partial credit for resume.

## Refresh checkpoint

After a successful refresh, append one line to `design/ingest/PROGRESS.md` in the form
`refresh <templateCommit>`, using the full current framework commit recorded in `design/kit.json`.
This is separate from the generation phase table and preserves its phase history.
